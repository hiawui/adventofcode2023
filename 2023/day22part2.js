...
let checkFallCnt = (brick, falled) => {
  ...
  for (const upId of Object.keys(brick.support)) {
    ...
    const supported = Object.keys(upBrick.supported).filter(
      (id) => !(id in falled)
    );
  ...
  for (const fId of Object.keys(newFall)) {
    cnt += checkFallCnt(bricks[fId], falled);
  }
  return cnt;
};
