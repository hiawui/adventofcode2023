...
let maxConsecutiveCnt = 10;
let minConsecutiveCnt = 4;
let maxI = null;
let maxJ = null;
let map = {};
input
...
for (let i = 0; i <= maxI; ++i) {
  for (let j = 0; j <= maxJ; ++j) {
    for (const [di, dj] of [
      ...
      for (let dc = 1; dc <= maxConsecutiveCnt; ++dc) {
...
do {
  ...
  for (const [di, dj] of [
    ...
    if (task.di === di && task.dj === dj && task.dc >= maxConsecutiveCnt) {
      continue;
  ...
  if (task.i === maxI && task.j === maxJ && task.dc >= minConsecutiveCnt) {
    break;
  }
} while (task);
if (task) {
